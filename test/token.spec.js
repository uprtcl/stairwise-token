...
const FUNDS_NOT_ENOUGH = "funds sent not enough";
...
contract("StaircaseBondingCurve", (accounts) => {
  ...
  it("should redeploy once again", async () => {
    tokenTemp4 = await StaircaseBondingCurve.new(
      ZERO_ADDRESS,
      steps,
      pricesM,
      initSupply,
  ...
  it("should be correctly configured", async () => {
    const stepsRead = await tokenTemp4.getSteps();
    ...
    const pricesMRead = await tokenTemp4.getPricesM();
    ...
    const supply = await tokenTemp4.totalSupply();
    assert.equal(supply.toString(), initSupply.toString(), "supply");

    const beneficiary = await tokenTemp4.beneficiary();
    assert.equal(beneficiary, execDAO, "beneficiary");

    const baseToken = await tokenTemp4.baseToken();
